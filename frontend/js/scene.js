...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  if (margins.minX > 0) {
    ...
    intervals.forEach(([a, b]) => {
      ...
      const suppressWall = suppressPoolFacingBridgeWall && snapSide === 'right';
      if (!suppressWall) addWallStrip(wallThickness, span, wallX, mid);
  ...
  if (margins.maxX > 0) {
    ...
    intervals.forEach(([a, b]) => {
      ...
      const mid = (a + b) * 0.5;
      addFloorStrip(floorWidth, span, floorCenterX, mid);
      const suppressWall = suppressPoolFacingBridgeWall && snapSide === 'left';
      if (!suppressWall) addWallStrip(wallThickness, span, wallX, mid);
      addCopingStrip(copingRebuildWidth, span, copingX, mid);
    });
  }
  if (margins.minY > 0) {
    ...
    intervals.forEach(([a, b]) => {
      ...
      const suppressWall = suppressPoolFacingBridgeWall && snapSide === 'back';
      if (!suppressWall) addWallStrip(span, wallThickness, mid, wallY);
  ...
  if (margins.maxY > 0) {
    ...
    intervals.forEach(([a, b]) => {
      ...
      const mid = (a + b) * 0.5;
      addFloorStrip(span, floorWidth, mid, floorCenterY);
      const suppressWall = suppressPoolFacingBridgeWall && snapSide === 'front';
      if (!suppressWall) addWallStrip(span, wallThickness, mid, wallY);
      addCopingStrip(span, copingRebuildWidth, mid, copingY);
    });
  }
